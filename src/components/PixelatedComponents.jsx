...
export const PixelButton = ({ 
  ...
  const variants = {
    ...
    accent: { bg: COLORS.dark, border: COLORS.brightOrange, text: COLORS.light },
  ...
  const baseStyles = {
    ...
    backgroundColor: filled 
      ? (scheme.bg === COLORS.dark ? COLORS.darkVariant : COLORS.lightVariant)
      : 'transparent',
  ...
  const handleMouseEnter = (e) => {
    if (disabled) return;
    const hoverBg = scheme.bg === COLORS.dark ? COLORS.darkVariant : COLORS.darkerVariant;
    e.currentTarget.style.backgroundColor = filled ? COLORS.darkerVariant : hoverBg;
  };
...
export const PixelCard = ({ 
  ...
  const handleMouseUp = (e) => {
    if (!onClick) return;
    e.currentTarget.style.transform = isActive ? 'translate(-1px, -1px)' : 'none';
...
export const PixelBadge = ({ 
  ...
  const variants = {
    primary: { bg: COLORS.darkVariant, border: COLORS.orange, text: COLORS.orange },
    secondary: { bg: COLORS.lightVariant, border: COLORS.teal, text: COLORS.teal },
  ...
  const handleMouseEnter = (e) => {
    e.currentTarget.style.backgroundColor = scheme.bg === COLORS.darkVariant ? '#151b22' : COLORS.darkerVariant;
  };
